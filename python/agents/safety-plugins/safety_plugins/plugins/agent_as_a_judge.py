...
_GOOGLE_MODEL_NAME_RESPONSE_REMOVED_MESSAGE = (
...
class JudgeOn(str, enum.Enum):
    ...
    GOOGLE_MODEL_NAME_OUTPUT = "model_output"
...
class LlmAsAJudge(BasePlugin):
    ...
    async def after_model_callback(
        ...
        if JudgeOn.GOOGLE_MODEL_NAME_OUTPUT not in self._judge_on:
            return None
        ...
        if await self._is_unsafe(message):
            return types.Content(
                role="model",
                parts=[types.Part.from_text(text=_GOOGLE_MODEL_NAME_RESPONSE_REMOVED_MESSAGE)],
            )
